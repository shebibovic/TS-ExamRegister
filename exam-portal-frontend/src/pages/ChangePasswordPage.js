...
const ChangePasswordPage = () => {
    ...
    const submitHandler = async (e) => {
        ...
        if (isValid) {
            ...
            try {
                const response = await fetch('http://10.0.130.222:8081/api/change-password/' + changeCode, {
                    method: 'POST',
                    headers: {
                ...
                if (response.status === 200) {
                    ...
                    swal("Password successfully changed!", responseData, "success")
                    navigate("/");

                } else {
                    const errorData = await response.text(); // Dobavljanje greške kao teksta
                    swal("Password change error!", errorData, "error")
                    navigate("/");
